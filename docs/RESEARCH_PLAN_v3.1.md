...
7. **GPU 饱和原则（2026-07-12 新增，用户要求）**：瓶颈是人工往返而非 GPU 时长——**每一轮结束时 GPU 上必须留有能跑数天的长任务队列**（基线复现、训练、扫参、消融），交互性调试与长训练并行。CPU 本地开发是为了缩短调试循环，不是为了省 GPU。排期上"卡等代码"是事故，"代码等卡"才是常态。
