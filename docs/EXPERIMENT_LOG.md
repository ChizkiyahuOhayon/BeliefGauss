...
**战略修正（2026-07-12，用户指示）**：GPU 饱和原则入约（计划 §3.3 第 7 条）——GPU 时长充裕、人工往返才是瓶颈，此后每轮必须给 GPU 留数天长任务。首个饱和任务：**OccWorld 基线复现即刻开训**（读源码确认其训练只读 Occ3D gts + pkl，不需要图像，不用等 trainval 下载）：官方权重先验证评测链路 → VQVAE → OccWorld 两阶段从头训（表 1 基线 + Plan C 被试）。GPU 排班：卡1 交互/提取、卡2 OccWorld 训练、卡3 预留 belief 训练（R5 起）、卡0 与机主协调。注意 CIFS 不能建软链 + labels.npz 高频随机读 → gts 拷本地盘。
